typedef unsigned char u8;typedef unsigned short u16;typedef struct Child{char p[0x6A];u8 index;}Child;typedef struct{Child*ptr;char p4[0x12];u16 flags;char tail[4];}Entry;typedef struct{Child*ptr;}Source;
