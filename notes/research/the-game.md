...
`tools_src/decode_tables.py` and extractor `tools_src/extract_mrg_tables.py`, both in `MaChInEgUn3/ygofm-decomp`).
...
| `+0x46000` | 0x16000 | `0x80146000` | the 90 KB **code overlay** (`docs/DISC.txt` in `MaChInEgUn3/ygofm-decomp`) |
...
* `0x80168000` is an overlay slot: different menu blobs load different code
  ...
  `gameshark-codes.md`). The overlay the Free Duel unlock patch and the
